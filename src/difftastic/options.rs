...
fn leaked_default(value: impl Display) -> &'static str {
    ...
    Command::new("Difftastic")
        ...
        .arg(
            Arg::new("tab-width")
                ...
                .env("DFT_TAB_WIDTH")
                .default_value(leaked_default(DEFAULT_TAB_WIDTH))
                .value_parser(clap::value_parser!(usize))
                .required(false),
        )
        .arg(
...
$ export DFT_OVERRIDE_BINARY_2='foo.pickle'"))
        ...
        .arg(
            Arg::new("byte-limit").long("byte-limit")
                ...
                .default_value(leaked_default(DEFAULT_BYTE_LIMIT))
                .env("DFT_BYTE_LIMIT")
                .value_parser(clap::value_parser!(usize))
                .required(false),
        )
...
Higher values will allow difftastic to perform a structural diff in more cases. Higher values will also increase the time before difftastic gives up on structural diffing, and increase peak memory usage.")
                .default_value(leaked_default(DEFAULT_GRAPH_LIMIT))
                .action(ArgAction::Set)
                .env("DFT_GRAPH_LIMIT")
                .value_parser(clap::value_parser!(usize))
                .required(false),
        ...
        .arg(
            Arg::new("parse-error-limit").long("parse-error-limit")
                ...
                .default_value(leaked_default(DEFAULT_PARSE_ERROR_LIMIT))
